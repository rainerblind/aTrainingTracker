...
| **REQ-UI-041** | Composable Device Management. | The entire device management interface (Tabs, List, and Edit Dialogs) SHALL be implemented using Jetpack Compose to ensure visual consistency and high performance. | `ListDeviceFragment.kt`, `DevicesTabbedContainerFragment.kt`, `EditGeneralDeviceFragment.kt` | `TST-UI-043` | Verified |
| **REQ-UI-042** | Mappable Device Items. | Individual device items SHALL use the `MappableListItem` foundation. The layout SHALL be organized into a multi-row structure: Header Row (54dp/44dp icon next to Name and Value/Status Icon), Manufacturer Row (next to icon), Battery Row (starting on the very left with an 18dp icon), Status Row (Connection status starting on the very left with relative \"last seen\" time and a 16dp icon), and Equipment Row. The pairing switch SHALL be positioned in the bottom-right corner. Internal phone sensors SHALL use a reduced 44dp icon size (vs 54dp for external sensors) and a muted 0.6f alpha on their icons to ensure technical subordination. | `DeviceItem.kt` | `TST-UI-044` | Verified |
| **REQ-UI-043** | Composable Edit & Selection Dialogs. | Device configuration and type selection SHALL be handled via Composable dialogs. The selection interface SHALL include device-specific icons (32dp) and high-readability labels to ensure a consistent visual identity with the primary list. | `EditDeviceDialog.kt`, `DevicesTabbedScreen.kt` | `TST-UI-043` | Verified |
