...
| **REQ-UI-042** | Mappable Device Items. | Individual device items SHALL use the `MappableListItem` foundation. The layout SHALL be organized into a multi-row structure: Header Row (54dp/44dp icon next to Name and Value/Status Icon), Manufacturer Row, Technical Status Row (18dp battery icon, percentage, and connection state in brackets), and Equipment Row. The pairing switch SHALL be positioned at the absolute bottom-right corner, using visual offsets and a 0.8f scale to minimize whitespace and prevent overlap with technical labels. | `DeviceItem.kt` | `TST-UI-044` | Verified |
