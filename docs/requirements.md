...
| **REQ-UI-042** | Mappable Device Items. | Individual device items SHALL use the `MappableListItem` foundation. The layout SHALL be organized into a multi-row structure: Header Row (Icon, Name, and Value), Manufacturer Row, Status Row (Connection status and Battery percentage starting below the icon), and Equipment Row. The pairing switch SHALL be positioned in the bottom-right corner. | `DeviceItem.kt` | `TST-UI-044` | Verified |
