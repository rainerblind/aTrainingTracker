...
4.  **Jira Ticket Management & Dual-Agent Workflow (Agile Phase)**:
    ...
    *   **MANDATORY TURN SEPARATION & PROHIBITION ON MULTI-STAGE COMPRESSION**:
        ...
        * **Automated Dual-Agent Workflow (Agent 1 Execution -> Automated Agent 2 Review -> Human Gate)**:
          Within every lifecycle stage (Stage 1 RCA, Stage 2 Planning, Stage 3 Implementation), development proceeds via a strictly segregated dual-agent workflow with human decision gates:
          1. **Phase 1 (Creation / Execution - Agent 1)**: Agent 1 creates the dedicated sub-task, transitions it to `In Bearbeitung`, performs the technical work (investigates logs/traces for RCA, formulates the plan artifact, or implements code and runs tests), sets the full documentation/artifact as the ticket/sub-task Description, and transitions the sub-task to `In Überprüfung`.
          2. **Phase 2 (Automated Independent Audit - Agent 2)**: Whenever Agent 1 completes its job and moves the sub-task to `In Überprüfung`, Agent 2 (Independent Senior Auditor) automatically conducts the formal Gate Review (Gate 1 RCA Audit, Gate 2 Plan Review, or Gate 3 Code Audit), verifies call sites and system invariants, posts the detailed audit report comment in Jira, and transitions the sub-task to `Freigabe (Human)`.
          3. **Phase 3 (Mandatory Human Decision Gate - Human User)**: In `Freigabe (Human)`, the agent MUST STOP and await human approval. Moving any sub-task or ticket to `Erledigt` is an inviolable human-only gate. The agent is strictly FORBIDDEN from starting the next lifecycle stage until the user has verified and transitioned the preceding sub-task to `Erledigt` in Jira.
    ...
    *   **Type-Aware Engineering & 3-Stage Dual-Agent Lifecycle**:
        ...
        *   **Stage 1: Root Cause Analysis (`[RCA]`)**:
            ...
            2.  **Phase 1 (Forensic Investigation - Agent 1)**: Transitions sub-task from `Zu erledigen` to `In Bearbeitung`. Analyzes logs, traces, and code paths. Formulates technical root cause vs. symptoms and corrective concept. Sets the complete RCA as the ticket's **Description**. Transitions sub-task to `In Überprüfung`.
            3.  **Phase 2 (Automated Forensic Audit - Agent 2)**: Automatically triggered upon sub-task entering `In Überprüfung`. Agent 2 (Independent Senior Forensic Auditor) conducts the Gate 1 review (scrutinizes findings, call sites, and system invariants). Posts the Gate 1 review report as a comment in Jira. Transitions sub-task to `Freigabe (Human)`.
            4.  **MANDATORY HARD STOP 1 (Human RCA Approval Gate)**: In `Freigabe (Human)`, the user reviews the analysis (in ticket Description) and audit comments:
        ...
        *   **Stage 2: Implementation Planning (`[Plan]`)**:
            ...
            3.  **Phase 1 (Architectural Planning - Agent 1)**: Transitions sub-task to `In Bearbeitung`. Formulates the plan at `docs/engineering/plans/ATT-XXX_plan.md`, maps requirements, affected components, and tests. Sets the full plan as the ticket's **Description**. Transitions sub-task to `In Überprüfung`.
            4.  **Phase 2 (Automated Plan Audit - Agent 2)**: Automatically triggered upon sub-task entering `In Überprüfung`. Agent 2 (Plan Auditor) audits the plan for architectural integrity, invariant safety, and complete test coverage (Gate 2 Plan Review). Posts the plan review report as a comment in Jira. Transitions sub-task to `Freigabe (Human)`.
            5.  **MANDATORY HARD STOP 2 (Human Plan Approval Gate)**: In `Freigabe (Human)`, the user reviews the plan (in ticket Description) and audit comments:
        ...
        *   **Stage 2: Implementation Planning (`[Plan]`)**:
            ...
            3.  **Phase 1 (Architectural Planning - Agent 1)**: Transitions sub-task to `In Bearbeitung`. Formulates the plan at `docs/engineering/plans/ATT-XXX_plan.md`, maps requirements, affected components, and tests. Sets the full plan as the ticket's **Description**. Transitions sub-task to `In Überprüfung`.
            4.  **Phase 2 (Automated Plan Audit - Agent 2)**: Automatically triggered upon sub-task entering `In Überprüfung`. Agent 2 (Plan Auditor) audits the plan for architectural integrity, invariant safety, and complete test coverage (Gate 2 Plan Review). Posts the plan review report as a comment in Jira. Transitions sub-task to `Freigabe (Human)`.
            5.  **MANDATORY HARD STOP 2 (Human Plan Approval Gate)**: In `Freigabe (Human)`, the user reviews the plan and audit comments:
                *   *Approve*: User moves sub-task to `Erledigt` (via *"Freigabe erteilt"*). **Agent 1 is strictly FORBIDDEN from writing any production code until the user has approved the plan.**
                *   *Reject / Revise*: User moves sub-task back to `In Bearbeitung` with revision requests.
        ...
        *   **Stage 3: Implementation & Verification (`[Impl]`)**:
            ...
            3.  **Phase 1 (Implementation & Testing - Agent 1)**: Transitions sub-task to `In Bearbeitung`. Implements the approved changes, ensures KDoc headers and 9-language localization, and executes unit/integration tests (`docs/tests.md`). Sets the walkthrough and test verification evidence as the ticket's **Description**. Transitions sub-task to `In Überprüfung`.
            4.  **Phase 2 (Automated Code Audit - Agent 2)**: Automatically triggered upon sub-task entering `In Überprüfung`. Agent 2 (Code Auditor) conducts Gate 3 review (`git diff` scrutiny, side-effects, localization compliance, invariant check, and full-suite regression run `./gradlew testDebugUnitTest`). Posts the Gate 3 audit report as a comment in Jira. Transitions sub-task to `Freigabe (Human)`.
            5.  **MANDATORY HARD STOP 3 (Human Implementation Approval Gate)**: In `Freigabe (Human)`, the user reviews the walkthrough and code audit comments:
                *   *Approve*: User moves sub-task to `Erledigt` (via *"Freigabe erteilt"*).
