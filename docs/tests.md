...
| **TST-UI-044** | **Device Item Visual Consistency** | `REQ-UI-042` | Compare DeviceItem against SegmentItem. Verify high-density layout: Top row (Name, Value), Manufacturer row, Status Line (16dp icon, available/last seen text), and Battery Line (18dp icon/%) starting on far left below 44dp device icon. Ensure 0.8f scaled right-aligned Switch. | Verified |
