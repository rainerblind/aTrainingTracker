...
| **TST-UI-049** | **Sensor Source Dialog Verification** | `REQ-UI-049` | 1. Start a session with at least one active hardware sensor (e.g., HR strap). 2. Tap the HR icon in the header. | A dialog SHALL appear showing the name of the HR strap, its technical identity (logo), the current heart rate value with units, and the battery level. | Verified |
| **TST-UI-050** | **Sensor Identity Consistency** | `REQ-UI-050` | Open both `SensorSourceDialog` and `EditDeviceDialog`. | Both SHALL feature identical technical identity blocks: Icon on left spanning Name and Battery rows, with Manufacturer left-aligned below the block. | Proposed |
