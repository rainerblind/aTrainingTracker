...
| **TST-UI-044** | **Device Item Visual Consistency** | `REQ-UI-042` | Compare DeviceItem against SegmentItem. Verify high-density layout: Top row (Name, Value), Manufacturer row, Status Line (Icon, available/last seen text, battery icon/%) below 54dp device icon. Ensure no horizontal divider and 0.8f scaled right-aligned Switch. | Verified |
