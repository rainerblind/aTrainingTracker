...
| **TST-UI-050** | **Sensor Identity Logic** | `REQ-UI-050` | Open both `SensorSourceDialog` and `EditDeviceDialog`. | Both SHALL feature identical Row 1/2 blocks. `EditDeviceDialog` SHALL include Row 3 (Manufacturer), while `SensorSourceDialog` SHALL omit it for density. | Verified |
