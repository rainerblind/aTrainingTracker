...
| **TST-UI-044** | **Device Item Visual Consistency** | `REQ-UI-042` | Compare DeviceItem against SegmentItem. Verify high-density layout: Top row (Name, Value), Manufacturer row, Technical Status Line (18dp battery icon, %, and connection state in brackets) starting on far left. Ensure 0.8f scaled right-aligned Switch. | Verified |
