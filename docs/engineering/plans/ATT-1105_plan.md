...
1. **Strava Default Name Recognition (`isDefaultStravaName`)**:
...
2. **Conditional Local DB Enrichment in `doUpdate()`**:
...
3. **Resilient Duplicate Detection in `checkAndUpdateDuplicate()`**:
...
4. **Comprehensive Catch Points in `uploadWorkout()`**:
...
5. **Reactive UI State Synchronization**:
...
- `app/src/test/java/com/atrainingtracker/trainingtracker/exporter/uploader/StravaUploaderNamingTest.kt`:
  - Update and expand test suite covering all 8 procedures of `TST-EXP-005`.
