...
2. **Dual-Perspective Presentation (`KnownLocationsScreen.kt`)**:
   ...
   - *List Perspective*: Renders a scrollable `LazyColumn` of known location cards. Each card SHALL display:
     ...
     - Source Badge reflecting provenance (`INTERNET_DEM`, `MANUAL_USER`, `AUTO_LEARNED`, `GPS_FALLBACK`, `LEGACY_RAW`) with localized label and distinct semantic tint.
     ...
     - Instant Altitude Lock toggle switch/icon reflecting `is_locked` (0 vs 1). Toggling the switch SHALL immediately commit `is_locked` to SQLite without mutating altitude or coordinates.
...
3. **Modal Edit Dialog (`EditKnownLocationDialog.kt`)**:
   ...
   - *Altitude Lock Switch*: Toggle switch controlling `is_locked`. When enabled (`is_locked = 1`), reference altitude is locked against automated background updates and DEM sweeps.
   - *"Fetch from Internet (DEM)" Action*: Prominent button allowing on-demand elevation retrieval via `ElevationService.fetchElevation(lat, lon)`. While fetching, an inline progress indicator is displayed. Upon success, the altitude field updates to the DEM elevation, `source` is set to `INTERNET_DEM`, and the lock switch remains optional.
   - *Action Bar*: Integrates `AppDialogActions.SaveCancel` with borderless "Abbrechen" (`@string/Cancel`) and filled "Speichern" (`@string/save`). Saving commits modifications to SQLite via `KnownLocationsRepository`. When the user manually edits the altitude value, `is_locked` SHALL automatically be set to `1` and `source` set to `MANUAL_USER`.
...
* **AC-2 (List View Presentation & Badges)**:
  ...
  - *Then* each card SHALL display Name, formatted Altitude with units, Source badge, Hit Count badge, 5-decimal coordinates, and an instant Lock toggle.
* **AC-3 (Instant Lock Toggle)**:
  - *Given* an unlocked location (`is_locked = 0`),
  - *When* the athlete toggles the Lock switch directly on the card,
  - *Then* `KnownLocationsRepository` SHALL persist `is_locked = 1` immediately to SQLite without modifying coordinates or altitude.
* **AC-4 (Manual Altitude Calibration & Lock)**:
  ...
  - *Then* SQLite SHALL update `altitude = 525.5`, `is_locked = 1`, and `source = MANUAL_USER`.
* **AC-5 (Online DEM Elevation Retrieval)**:
  - *Given* an athlete tapping "Fetch from Internet (DEM)" in the edit dialog,
  - *When* the API returns elevation 523.0m,
  - *Then* the altitude field SHALL update to 523.0m and source SHALL update to `INTERNET_DEM`.
...
| **TST-UI-117.3** | `KnownLocationsViewModelTest.kt` | Call `toggleLock(locationId, isLocked = true)`. | Database row `is_locked` updates to 1; StateFlow emits updated item; altitude and coordinates remain identical. | `is_locked == 1`, altitude unchanged. |
...
| **TST-UI-117.5** | `KnownLocationsScreenTest.kt` | Render `KnownLocationsScreen` in List mode. | Cards display Location Name, Altitude (m/ft), Source badge, Hit Count badge, coordinates (5 decimals), and Lock toggle. | All card elements rendered and verified. |
...
| **TST-UI-117.10** | `TranslationParityTest.kt` | Audit all 20 new string resources across all 9 locales (EN, DE, ES, FR, IT, JA, NL, PL, PT) for keys: `drawer_start_locations`, `known_locations_title`, `known_locations_tab_list`, `known_locations_tab_map`, `known_locations_search_hint`, `known_locations_empty_title`, `known_locations_empty_desc`, `known_location_unnamed_format`, `known_locations_starts_count`, `known_locations_lock_title`, `known_locations_lock_desc`, `known_locations_locked_badge`, `known_locations_unlocked_badge`, `source_internet_dem`, `source_manual_user`, `source_auto_learned`, `source_gps_fallback`, `source_legacy_raw`, `known_location_edit_title`, `known_location_fetch_dem`. | 0 missing translation keys, 100% localization parity across 9 locales. | Full parity across 9 locales. |
