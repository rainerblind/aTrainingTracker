...
public class WorkoutDetailsActivity extends AppCompatActivity
    ...
    private static final String CALCULATING_EXTREMUM_VALUES = "CALCULATING_EXTREMUM_VALUES";
    private final IntentFilter mFinishedCalculatingExtremumValuesFilter = new IntentFilter(CalcExtremumValuesTask.FINISHED_CALCULATING_EXTREMUM_VALUES);
    ...
    long mWorkoutID;
    private boolean mCalculatingExtremumValues = false;
    private final BroadcastReceiver mFinishedCalculatingExtremumValuesReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            // remove the progress view
            findViewById(R.id.llProgress).setVisibility(View.GONE);
            mCalculatingExtremumValues = false;
        }
    };
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            mCalculatingExtremumValues = savedInstanceState.getBoolean(CALCULATING_EXTREMUM_VALUES, false);
        }
        ...
        if (mCalculatingExtremumValues) {  // currently calculating the extremum values
            findViewById(R.id.llProgress).setVisibility(View.VISIBLE);
        } else if (!WorkoutSummariesDatabaseManager.getBoolean(mWorkoutID, WorkoutSummaries.EXTREMUM_VALUES_CALCULATED)) {  // extremum values are not yet calculated
            Log.i(TAG, "calculate the extremum values");
            mCalculatingExtremumValues = true;

            // show the progress view
            findViewById(R.id.llProgress).setVisibility(View.VISIBLE);

            // now, calc the extremum values in the background
            (new CalcExtremumValuesTask(this, (TextView) findViewById(R.id.tvProgressMessage))).execute(mWorkoutID);
        }
    }
    ...
    public void onSaveInstanceState(Bundle savedInstanceState) {
        ...
        savedInstanceState.putBoolean(CALCULATING_EXTREMUM_VALUES, mCalculatingExtremumValues);
    ...
    protected void onPause() {
        ...
        try {
            unregisterReceiver(mFinishedCalculatingExtremumValuesReceiver);
