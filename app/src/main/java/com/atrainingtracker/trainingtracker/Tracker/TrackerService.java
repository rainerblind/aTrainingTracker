...
public class TrackerService extends Service {
    ...
    // finally the main tracking routine, that is called periodicallty
    ...
    protected void createNewLap() {
        if (DEBUG) Log.i(TAG, "creatNewLap");
    ...
    public void endWorkout() {
        ...
        // ARGH, when the service crashes, not only the flag is not set but the whole method is not executed.
    ...
    private void sampleAndWriteToDb() {
        ...
        summaryValues.put(WorkoutSummaries.EXTREMUM_VALUES_CALCULATED, 0);    // force recalculation of the extremum values in the case that they have been calculated
        if (averageSpeedCalculateable()) {
